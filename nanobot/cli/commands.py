...
def gateway(
    ...
    mcp_enabled = config.tools.mcp.enabled if hasattr(config.tools, 'mcp') else False
    if mcp_enabled:
        from nanobot.agent.mcp import MCP_AVAILABLE
        if not MCP_AVAILABLE:
            console.print("[yellow]Warning: MCP enabled but mcp package not installed[/yellow]")
            console.print("  Install with: pip install mcp")
        else:
            console.print(f"[green]✓[/green] MCP: {len(config.tools.mcp.servers)} servers configured")
    ...
    async def run():
        try:
            ...
            if mcp_enabled and hasattr(agent, 'mcp_client') and agent.mcp_client:
                try:
                    await agent.start_mcp()
                    console.print("[green]✓[/green] MCP connections established")
                except Exception as e:
                    logger.error(f"Failed to start MCP: {e}")
                    console.print(f"[yellow]Warning: MCP connection failed: {e}[/yellow]")
